...
export default function BookingTab({ trip, onUpdate }) {
  ...
  const [form, setForm] = useState({ category:'flight', title:'', detail:'', subDetail:'', status:'confirmed' })

  const grouped = CATEGORIES.map(c => ({
    ...c, items: trip.bookings.filter(b => b.category === c.key)
  })).filter(g => g.items.length > 0)

  function addBooking() {
    if (!form.title) return
    const updated = { ...trip, bookings: [...trip.bookings, { id: Date.now(), ...form }] }
    onUpdate(updated)
    setForm({ category:'flight', title:'', detail:'', subDetail:'', status:'confirmed' })
    setShowModal(false)
  }
      ...
      <div className="screen">
        {grouped.length === 0 && (
          <div style={{textAlign:'center',color:'var(--gray-400)',padding:'40px 0',fontSize:14}}>
            예약 내역이 없어요<br/>
            <span style={{fontSize:12}}>위 추가 버튼으로 등록해보세요</span>
          </div>
        )}
        {grouped.map(g => (
          <div key={g.key}>
            ...
            {g.items.map(item => {
              ...
              return (
                <div key={item.id} className="card" style={{display:'flex',gap:10,alignItems:'flex-start'}}>
                  <div style={{width:38,height:38,borderRadius:9,background:g.bg,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0}}>
                    {g.icon}
                  </div>
                  <div style={{flex:1}}>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()}>
            <div className="modal-title">예약 추가</div>
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="예: 아시아나 OZ111" value={form.title} onChange={e => setForm({...form,title:e.target.value})} />
            </div>
            <div className="form-group">
              <label className="form-label">날짜 / 상세</label>
              <input className="form-input" placeholder="예: 6/3 ICN→KIX 08:30" value={form.detail} onChange={e => setForm({...form,detail:e.target.value})} />
            </div>
            <div className="form-group">
              <label className="form-label">예약번호 / 메모</label>
              <input className="form-input" placeholder="예: 예약번호 ABCD12" value={form.subDetail} onChange={e => setForm({...form,subDetail:e.target.value})} />
            </div>
            <div className="form-group">
            ...
            <button className="btn-primary" onClick={addBooking}>추가하기</button>
          </div>
        </div>
      )}
    </div>
